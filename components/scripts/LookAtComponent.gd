...
func update_rotation_with_angle(angle: float,delta: float):
	print(angle)
	current_rotation.y = lerp_to_angle(current_rotation.y,current_rotation.y+angle,delta)
...
func update_rotation_with_target_position(target_to_look: Vector3,delta: float):
